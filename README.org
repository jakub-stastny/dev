...
My development environment with my favourite tools and my dotfiles.
...
There's also [[./server.install.org][server.install.org]] for [[VPS setup][setting up the host server]].
