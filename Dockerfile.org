  ...
  (defn format-duration [duration lambda]
    (if (<= duration 3) (println)
      (cond
        ...
        (> duration 60) (lambda (str (int (/ duration 60.0)) ":" (format "%02d" (mod duration 60)) "m")))))
    ...
    (let [start-time (current-unix-time)]
      ...
      (let [result (zsh command rest)]
        (let [duration (- (current-unix-time) start-time)]
          (format-duration duration (fn [formatted-duration]
            (println  "... took" (str (colour :green formatted-duration) ".")))) result))))
  ...
  (defmacro block [name & sexps]
    `(do
      ...
      (let [start-time (current-unix-time)]
        (do ~@sexps)
        (let [duration (- (current-unix-time) start-time)]
          (println (colour :cyan "\n  ~") "Block" (colour :grey ~name) "took"
            (format-duration duration (fn [format-duration]
              (str (colour :yellow (str formatted-duration "s")) ".\n"))))))))
