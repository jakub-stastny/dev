...
#+BEGIN_SRC shell :tangle scripts/install :mkdirp yes :shebang #!/bin/sh
  ...
  apt-get install -y zsh apt-utils locales build-essential automake htop curl wget git silversearcher-ag neovim docker.io tmux tree > /dev/null
#+END_SRC

** Install Emacs 28 with native compilation and fast JSON parser

#+BEGIN_SRC shell :tangle scripts/install :mkdirp yes :shebang #!/usr/bin/env zsh
