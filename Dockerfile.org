...
#+name: bb-helpers
#+begin_src clojure
  (require '[clojure.java.shell :refer [sh]]) ; rename to exec or sth

  (defn exec [command]
    (let [chunks (clojure.string/split command #" ")]
      (apply sh chunks)))

  (defn zsh [command]
    (apply sh "zsh" "-c" command))
  ...
  (defn package [& names]
    (ensure-success (run (str "apt-get install -y " (clojure.string/join " " names)))))
#+end_src
