...
#+begin_src shell :tangle scripts/unminimise-system
  ...
  unminimize-non-interactively() { time (yes | unminimize) }
  run-hide-stdout unminimize-non-interactively
#+end_src
...
#+begin_src shell :tangle scripts/build-emacs :mkdirp yes :shebang #!/usr/bin/env zsh
  ...
  run-hide-stdout time git clone --depth 1 https://git.savannah.gnu.org/git/emacs.git
  ...
  package build-essential texinfo
