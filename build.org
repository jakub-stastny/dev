...
#+begin_src clojure :tangle scripts/build-emacs
  ...
  (block "building Emacs 28 with native compilation and fast JSON"
    (with-sh-dir "emacs"
      (let [env {:path (System/getenv "PATH") :cc "gcc-10"}]
        (ensure-success (run "./autogen.sh"))
        (ensure-success (run "./configure --with-native-compilation" :env env))
...
#+begin_src clojure :tangle scripts/add-node-sources
  ...
  (block "adding apt sources for Node.js"
    (package "gnupg")
    (let [get-key "curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg"
          add-key "apt-key add -"
          echo-source "echo deb https://dl.yarnpkg.com/debian/ stable main"
          add-source-list-entry "tee /etc/apt/sources.list.d/yarn.list"]
      (ensure-success
       (run (str get-key " | " add-key " && " echo-source " | " add-source-list-entry)))))
#+end_src
...
#+begin_src clojure :tangle scripts/set-up-time-zone
  ...
  (block "setting up time zone"
    (let [time-zone "America/Mexico_City"]
      (ensure-success
...
#+begin_src clojure :tangle scripts/install-clojure-cli
  ...
  (block "installing Clojure CLI"
    (let [version "1.10.3.933"    ;  July 28, 2021
          script-name "linux-install.sh"
          script-url (str "https://download.clojure.org/install/linux-install-" version ".sh")]
      (ensure-success (run (str "curl " script-url " -o " script-name)))
...
#+begin_src clojure :tangle scripts/install-gh-cli
  ...
  (block "installing GitHub CLI"
    (let [get-key "curl -fsSL https://cli.github.com/packages/githubcli-archive-keyring.gpg"
      ...
      (ensure-success (run (str get-key " | " gpg-dearmor)))
      (ensure-success (run (str source-list-entry "' | tee " source-list-path)))
...
#+begin_src clojure :tangle scripts/make-zsh-the-default-shell
  ...
  (block "making ZSH the default shell"
    (let [zsh-path (str/trim (:out (ensure-success (run "which zsh"))))]
      (ensure-success (run (str "chsh -s " zsh-path)))))
#+end_src
...
#+begin_src clojure :tangle scripts/secure-installation
  ...
  (block "setting up OpenSSH server and securing the installation"
    (let [random-password (random-string 32)]
      ...
      (ensure-success (run (str "echo 'root:" random-password "' | chpasswd")))))
#+end_src

Alternatively we could use: =(ensure-success (run "echo \"root:$(tr -dc A-Za-z0-9 </dev/urandom | head -c 32)\" | chpasswd"))=.
...
#+begin_src shell :tangle scripts/verify-installation :shebang "#!/usr/bin/env zsh"
