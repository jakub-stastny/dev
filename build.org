...
#+begin_src clojure
  ...
  (defn- format-duration [duration]
    (if-let [colour-name
             (cond (< duration (:instant execution-duration-limits)) nil
                   ...
                   :default :red)]
      (colour colour-name (cond
                            (< duration 60) (format-duration-as-seconds duration)
                            (> duration 60) (format-duration-as-minutes duration)))))
  ...
  (defn- format-duration-unless-instant [duration formatter-fn]
    (if-let [formatted-duration (format-duration duration)]
      (formatter-fn formatted-duration) ""))

  (comment
    (format-duration-unless-instant 2 identity)
    (format-duration-unless-instant 7 identity))

  (defn- format-duration-wrapper
    ([duration] (format-duration-unless-instant duration #(str "took " (format-duration %) ".")))
    ([duration lambda] (format-duration-unless-instant duration #(lambda (format-duration-wrapper %)))))

  (comment
    (format-duration-wrapper 1)
    (format-duration-wrapper 37)
    (format-duration-wrapper 92))

  (defn run [& args]
    (let [[command] args start-time (current-unix-time)]
      (print (str "  " (colour :green "λ ") (colour :grey command)) "")
      (flush)
      (let [result (apply zsh args)]
