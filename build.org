...
#+begin_src clojure
  ...
  (defn counter [result-atom command & rest]
      ...
      (loop [duration (- (current-unix-time) start-timestamp)]
        (when-not (deref result-atom)
          ...
          (print (str "\r  "
                      ...
                      (colour :grey command)
                      " ... "
                      (format-duration-unless-instant duration)))
          (flush)
          (recur)))))
  ...
  (defn run [command & rest]
    (let [result-atom (atom nil)]
      (run! deref
            [(future (execution-worker result-atom command rest)),
             (future (counter result-atom command rest))])))
