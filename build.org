...
#+begin_src clojure
  ...
  (defn run [command & rest]
    (let [start-time (current-unix-time) result (atom nil)]
      (print (str "  " (colour :green "λ ") (colour :grey command)) "")
      (flush)

      ;; Trying to integrate this
      (run! deref
            [(future (execution-worker result)),
             (future (counter result))])
  ...
  (defn counter [result]
    (let [start-timestamp (current-unix-time)]
      (loop []
        (when-not (deref result)
          (Thread/sleep 1000)
          (print (str "\r" (- (current-unix-time) start-timestamp)))
          (flush)
          (recur)))))
