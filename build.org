...
*** TODO Copy bin/verify into the image

We can then use it for tests etc.

Or maybe even run it as the last thing, if it fails, the image build fails.

No need for GH actions then.
