...
* TODO Should I have interactive (DPM) vs non-interactive (CI) image?
- jakubstastny/base & jakubstastny/dev
- Woukd be easy to do so with ~base-package~ vs. ~int-package~.
  - or just inherit the Dockerfile, even better
- Not sure whether it's necessary though. We need Emacs either way.
  htop, unminimise, gh-cli

#+name: build
#+begin_src sh :task yes :doc "Build the Docker image"
  # et build.org build
  docker build -t jakubstastny/build
#+end_src

#+name: push
#+begin_src sh :task yes :doc "Upload the built image to Docker Hub"
  # et build.org push
  docker push jakubstastny/build
#+end_src
...
#+begin_src clojure :tangle scripts/install-basic-tools
  ...
  (block "installing basic tools"
    (package "locales" "automake" "htop" "curl" "wget" "git" "silversearcher-ag"
             "neovim" "docker.io" "tmux" "tree" "expect-dev" "rlwrap" "git" "emacs-nox"))
#+end_src
...
#+begin_src clojure :tangle scripts/add-node-sources
  ...
  (run "apt-get update")
  (package "lsb-core") ; The distro helper requires this, it's not installed by default anymore.

  (def version "node_19.x")
  ...
  ; For now, working on BizMentor.
  (block "install Node.js" (package "nodejs" "npm"))

  ; HACK: it doesn't belong here!
  (block "install JRE & R" (package "default-jre" "r-base" "rsync"))
