...
*** TODO Consider what to do with shell commands and what with Babashka

On one hand shell commands are easier to copy around, on the other hand doing certain things with Babashka might be cleaner and we might not need so many =(str a b c)=.

Babashka has =curl= for instance.

Maybe we can do only things that cannot be done any other way (such as =apt-get install -y=) in shell?
...
*** FIXME If the line is too long, \r wouldn't work
...
#+begin_src clojure
  ...
  ; TODO: Redirect STDERR to STDOUT, so they would be in sync when mixing STDOUT/STDERR for logging.
  ...
  (comment
    (colour :red "Hello world!")
    (colour "whatever" "Hello world!"))
  ...
  (comment
    (format-duration 1)                   ; show nothing
    (format-duration 37)
    (format-duration 92))
  ...
  (comment
    (format-duration-unless-instant 2 #(str "~ " %))
  ...
  (comment
    (error-message {:exit 127 :err "Command not found."})
    (error-message {:exit 0 :out "Log message."})
    (error-message {:exit 1 :out "Log message." :err "Stack overflow"}))
  ...
  (defn fun [fn-meta fn & args]
    (print (str "\r  "
                (colour :green "λ ")
                (if (:doc fn-meta)
                  (colour :grey (format "(%s) ; %s" (:name fn-meta) (:doc fn-meta)))
                  (colour :grey (format "(%s)" (:name fn-meta))))))
    (fn))

  (comment
    (let [no-doc-fn ^{:name "no-doc-fn"} (fn [] "body")]
      (fun (meta #'no-doc-fn) no-doc-fn))
    (let [doc-fn ^{:name "doc-fn" :doc "Doc string"} (fn [] "body")]
      (fun (meta #'doc-fn) doc-fn)))
  ...
  (def distro (str/trim (:out (ensure-success (zsh "lsb_release -s -c")))))
...
#+begin_src clojure :tangle scripts/install-basic-tools
  ...
  (block "installing basic tools"
    (package "locales" "automake" "htop" "curl" "wget" "git" "silversearcher-ag"
             "neovim" "docker.io" "tmux" "tree" "expect-dev" "rlwrap"))
#+end_src

** Node.js & Yarn sources

This script used to add Yarn sources as well, but starting from Yarn 2, the recommended way is to install Yarn per each project rather than system-wide. See the [[https://yarnpkg.com/getting-started/install][Yarn installation guide]].
...
#+begin_src clojure :tangle scripts/add-node-sources
  ...
  (def version "node_16.x")
  (def domain "https://deb.nodesource.com")

  (defn get-key "Get NodeSource GPG key" []
    (:body (curl/get (format "%s/gpgkey/nodesource.gpg.key" domain))))

  (defn write-source-list "Add a new APT source list for NodeSource" []
    (spit "/etc/apt/sources.list.d/nodesource.list"
          (str/join "\n"
                    [(format "deb %s/%s %s main" domain version distro)
                     (format "deb-src %s/%s %s main" domain version distro)])))

  (block "adding apt sources for Node.js"
    (let [gpg-key (fun (meta #'get-key) get-key)]
      (run "apt-key add -" :in gpg-key))
    (fun (meta #'write-source-list) write-source-list))
#+end_src
...
#+begin_src clojure :tangle scripts/set-up-time-zone
  ...
  ; TODO: write the TZ into /etc/timezone
  (block "setting up time zone"
    (run (str "ln -sf /usr/share/zoneinfo/" time-zone "/etc/localtime")))
#+end_src
...
Current release can be found on this page: [[https://clojure.org/releases/tools]].

#+begin_src clojure :tangle scripts/install-clojure-cli
  <<bb-helpers>>

  ; TODO: get this from GH or somewhere.
  (def version "1.10.3.933") ; July 28, 2021

  (block "installing Clojure CLI"
    (let [script-name "linux-install.sh"
          script-url (format "https://download.clojure.org/install/linux-install-%s.sh" version)]
      (run (format "curl %s -o %s" script-url script-name))
...
#+begin_src clojure :tangle scripts/secure-installation
  ...
  (block "setting up OpenSSH server and securing the installation"
    ...
    (fun (meta #'create-sshd-run-directory) create-sshd-run-directory)
    (fun (meta #'disable-password-auth-for-ssh) disable-password-auth-for-ssh)
    (fun (meta #'set-root-password-to-random) set-root-password-to-random))
#+end_src
