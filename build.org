...
#+name: bb-helpers
#+begin_src clojure
  ;; TODO: Show how much time is something taking.
  ...
  (def ^:private execution-duration-limits {:short 35 :normal 90})
  ...
  (defn- format-duration [duration]
    (let [colour-name (cond (< duration 35) :green
                            (< duration 90) :yellow
                            :else           :red)]
  ...
  ;; TODO: refactor, this should be in format-duration: return false if too short.
  (def ^:private too-short 3)
  (defn- format-duration-wrapper
    ([duration] (if (<= duration too-short) "" (str "took " (format-duration duration) ".")))
    ([duration lambda] (if (<= duration too-short) "" (lambda (format-duration-wrapper duration)))))
