...
#+begin_src clojure
  ...
  ;; (println (apply prn-str "zsh" "-c" args))
  ;; "zsh" "-c" "apt-get install -y man" (:env {:DEBIAN_FRONTEND "noninteractive"})
  (defn zsh [& args] (println (map type args)) (apply sh "zsh" "-c" args))
      ...
      (let [result (zsh command rest)]
        (let [duration (- (current-unix-time) start-time)]
          (let [format-fn (fn [formatted-duration-string] (str "... " formatted-duration-string))]
            (println (format-duration-unless-instant duration format-fn)))
          result))))
  ...
  (defmacro block
    ...
    `(do
       ...
       (let [start-time (current-unix-time)]
         (do ~@sexps)
         (let [duration (- (current-unix-time) start-time)]
           (println (colour :cyan "\n  ~") "Block" (colour :grey ~name) (format-duration-unless-instant duration #(str "took" % ".")) "\n")))))
